...
export class AuthService {
  ...
  async login({ username , password }): Promise<any>  {
    ...
    const data  = await this.http.get(environment['apiBaseUrl'].replace(/\/$/, '') + '/login', requestOptions).toPromise();

    // this part only gets executed when the promise is resolved
    if (data['client_id'] && data['client_session_id']) {
  ...
  private setDataAfterLogin(data) {
    this.token  = data['client_session_id'];
